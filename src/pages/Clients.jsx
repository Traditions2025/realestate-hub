...
const SMART_LISTS = [
  ...
  { key: 'fsbo_reaching_dom14', label: 'FSBO · Reaching 14 Days', desc: 'Available FSBOs at DOM 10-13 — the auto-campaign starts at DOM 14' },
  { key: 'fsbo_campaign_active', label: 'FSBO · Campaign Active', desc: 'FSBOs enrolled in the automatic text campaign with a future send' },
  { key: 'fsbo_campaign_responded', label: 'FSBO · Response Received', desc: 'FSBO campaign leads who replied — humans own these conversations' },
    ...
    <div style={{ marginTop: 10, border: '1px solid var(--border)', borderRadius: 10, padding: 12, background: 'var(--bg-secondary)' }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
        ...
        <button className="btn btn-sm btn-secondary" title="Insert the website link + branded preview image (the image always shows, even when their phone won't auto-load link previews)"
          onClick={() => {
            setBody(b => (b && b.trim() ? b.replace(/\s+$/, '') + '\n' : '') + 'https://mattsmithteam.com/')
            const cardUrl = window.location.origin + '/website-card.jpg'
            setMedia(list => list.some(m => m.url === cardUrl) ? list : [...list, { url: cardUrl, type: 'image/jpeg' }])
