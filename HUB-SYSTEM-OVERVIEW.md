...
- **Cadence (per John, 2026-09-14):** approved copy unchanged, order set by John — availability check (Text 1) → +7d still-available check (Text 2) → +7d the 35-years/first-14-days 3-part message (Text 3) — then an **availability check every 40+ days** (40–47-day jitter, Sat→Fri/Mon, Sun→Mon) from a 4-angle availability-only bank (no repeat within last 3; never "we have a buyer", never "how has activity been"). Persistence ≠ pressure. Identity safety: "the home at {address}", no first names in cold texts.
...
- **Response = STOP FIRST, NO AUTO-REPLY:** any inbound sets `responded`, kills the next send, logs, notifies, creates a high-priority **FSBO Response task** (one per conversation). Nothing ever replies automatically — every response is answered personally. Humans own it; no silent resume. An inbound on any channel since enrollment (email, voicemail) also stops it at the next evaluation.
