...
export function logMasterUpdate(clientId, list, change, detail) {
  try {
    ...
    db.run('INSERT INTO master_file_updates (client_id, client_name, list, change, detail, created_at) VALUES (?,?,?,?,?,?)',
      [Number(clientId), name, list, change, detail, new Date().toISOString()])
