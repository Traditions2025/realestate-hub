...
function fillMerge(text, c) {
  ...
  return String(text)
    ...
    .replace(/\{\{search_price_range\}\}/g, (c.search_price_min == 200000 && c.search_price_max == 600000) ? '' : priceRange)
    .replace(/\{\{price_point\}\}/g, (() => { try { const rows = db.all("SELECT prop_price FROM fub_activity WHERE client_id = ? AND prop_price IS NOT NULL AND prop_price != ''", [c.id]); const nums = rows.map(r => Number(String(r.prop_price).replace(/[^0-9.]/g, ''))).filter(n => n > 10000 && n < 20000000); if (!nums.length) return ''; return '$' + (Math.round((nums.reduce((a, b) => a + b, 0) / nums.length) / 10000) * 10000).toLocaleString() } catch { return '' } })())
