...
const SORT_OPTIONS = {
  ...
  // Follow-Up Coverage sorts (scalar subqueries against the coverage summary table)
  most_overdue: '(SELECT fc.overdue_by_days FROM followup_coverage fc WHERE fc.client_id=clients.id) DESC NULLS LAST',
  longest_since_contact: '(SELECT fc.days_since_contact FROM followup_coverage fc WHERE fc.client_id=clients.id) DESC NULLS LAST',
  coverage_risk: "(SELECT CASE fc.coverage_status WHEN 'unprotected' THEN 0 WHEN 'at_risk' THEN 1 WHEN 'snoozed' THEN 2 WHEN 'protected' THEN 3 ELSE 4 END FROM followup_coverage fc WHERE fc.client_id=clients.id) ASC NULLS LAST",
  next_action_soonest: '(SELECT fc.next_action_at FROM followup_coverage fc WHERE fc.client_id=clients.id) ASC NULLS LAST',
  highest_ai_intent: '(SELECT fc.intent_score FROM followup_coverage fc WHERE fc.client_id=clients.id) DESC NULLS LAST',
  ...
  last_email_out_recent: 'last_email_out DESC NULLS LAST',
  last_email_out_oldest: 'last_email_out ASC NULLS LAST',
...
const COMM_RECENCY_SELECT = `,
  ...
  (SELECT MAX(co.occurred_at) FROM communications co WHERE co.client_id=clients.id AND co.channel='email' AND co.direction='outgoing') AS last_email_out,
  ...
  (SELECT fc.coverage_status FROM followup_coverage fc WHERE fc.client_id=clients.id) AS coverage_status,
  (SELECT fc.next_action_at FROM followup_coverage fc WHERE fc.client_id=clients.id) AS fc_next_action_at,
  (SELECT fc.next_action_type FROM followup_coverage fc WHERE fc.client_id=clients.id) AS fc_next_action_type,
  (SELECT fc.days_since_contact FROM followup_coverage fc WHERE fc.client_id=clients.id) AS fc_days_since_contact,
  (SELECT fc.last_meaningful_contact_at FROM followup_coverage fc WHERE fc.client_id=clients.id) AS fc_last_meaningful_at`
...
// ---- FACEBOOK LISTING-AD LEAD INTAKE (Zapier → Hub, instant speed-to-lead) ----
// Public endpoint guarded by a shared key (?key=...). Zapier's "Facebook Lead Ads"
// trigger posts the form fields here the moment someone submits — the lead is in
// the Hub within seconds, tagged to the listing/campaign, and the AI fresh lane
// texts them minutes later. Dedupes against existing leads by phone, then email.
// ?dry=1 validates + parses without writing anything.
router.post('/fb-webhook', async (req, res) => {
  const expected = db.getSetting('fb_lead_webhook_key', '')
  if (!expected || String(req.query.key || req.body?.key || '') !== expected) return res.status(403).json({ error: 'bad key' })
  const b = req.body || {}
  ...
  let first = pick('first_name'), last = pick('last_name')
  if (!first && full) { const parts = full.split(/\s+/); first = parts[0]; last = parts.slice(1).join(' ') }
  const email = (pick('email') || null)
  const d10 = pick('phone_number', 'phone').replace(/\D/g, '').slice(-10)
  const phone = d10.length === 10 ? `(${d10.slice(0, 3)}) ${d10.slice(3, 6)}-${d10.slice(6)}` : null
  const timeline = pick('timeline', 'when_are_you_looking_to_move?', 'when_are_you_looking_to_move', 'custom_question')
  const listing = pick('listing', 'campaign_name', 'adset_name', 'form_name')
