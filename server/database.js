...
export async function initDb() {
  ...
  try { db.run('ALTER TABLE clients ADD COLUMN sms_line_type TEXT') } catch {}
  ...
  try { db.run('ALTER TABLE clients ADD COLUMN mls_list_price INTEGER') } catch {}  // Cancelled/Expired: last MLS list price, parsed from the master file Notes blob
  ...
  try {
    ...
    const newCols = [
      ...
      ['fsbo_status', 'TEXT'],
      ['fsbo_status_at', 'TEXT'],
      // FSBO master-file list date + days-on-market (shown in the FSBO list in place of
      // Registered and Type).
      ['fsbo_list_date', 'TEXT'],
      ['fsbo_dom', 'TEXT'],
      ['fsbo_notes', 'TEXT'],
